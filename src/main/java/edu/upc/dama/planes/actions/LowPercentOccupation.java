...
public class LowPercentOccupation implements Action, GraphAware {
...
//				weightCapacity = wieghtCapacity.getLong();
		...
		for (Entry<String, Long> entry : flight.entrySet()) {
			System.out.println(entry + " " + flightType.get(entry.getKey()) + " " + nbPassengerPercent.get(entry.getKey()));
